...
import { 
  ...
  LogOut, Settings, BarChart3, HelpCircle
} from 'lucide-react';
...
const Sidebar = () => {
  ...
  const getNavLinks = () => {
    ...
    if (role === 'PLATFORM_ADMIN') {
      links.push(
        { to: '/admin/entities', icon: <Building2 size={18} />, label: 'Entitas Bisnis' },
        { to: '/admin/users', icon: <Users size={18} />, label: 'Pengguna' },
        { to: '/admin/categories', icon: <Layers size={18} />, label: 'Kategori Produk' },
        { to: '/products', icon: <Package size={18} />, label: 'Semua Produk' },
      );
    }

    if (role === 'FARMER') {
      links.push(
        { to: '/products', icon: <Package size={18} />, label: 'Katalog Produk' },
        { to: '/batches', icon: <Tractor size={18} />, label: 'Batch Panen' },
        { to: '/inventories', icon: <Layers size={18} />, label: 'Inventaris Saya' },
        { to: '/transactions/sales', icon: <ArrowRightLeft size={18} />, label: 'Penjualan' },
        { to: '/waste', icon: <Trash2 size={18} />, label: 'Pencatatan Limbah' }
      );
    }

    if (role === 'CONSUMER') {
      links.push(
        { to: '/business', icon: <Building2 size={18} />, label: 'Profil Bisnis' },
        { to: '/inventories', icon: <Layers size={18} />, label: 'Gudang Inventaris' },
        { to: '/transactions/purchases', icon: <ShoppingCart size={18} />, label: 'Pembelian (Inbound)' },
        { to: '/transactions/sales', icon: <Store size={18} />, label: 'Penjualan (Outbound)' },
        { to: '/waste', icon: <Trash2 size={18} />, label: 'Limbah & Kadaluarsa' },
        { to: '/traceability', icon: <BarChart3 size={18} />, label: 'Lacak Produk' }
      );
    }
